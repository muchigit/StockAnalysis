import { useState, useEffect } from 'react';

type Lang = 'en' | 'ja';

const dictionary = {
    en: {
        dashboardTitle: "Investment Dashboard",
        automation: "Automation",
        runImport: "Run Import",
        importStarted: "Import started!",
        importCompleted: "Import completed!",
        importFailed: "Import failed",
        importPlaceholder: "C:\\path\\to\\export.xls",
        importing: "Importing...",
        noStocksFound: "No stocks found. Run import to get started.",
        loading: "Loading...",
        stockNotFound: "Stock not found",
        backToDashboard: "Back to Dashboard",
        priceChart: "Price Chart",
        noChartData: "No Chart Data",
        technicalSignals: "Technical Signals",
        analysisNotes: "Analysis / Notes",
        tradeHistory: "Trade History",
        noTradesFound: "No trades found.",
        date: "Date",
        side: "Side",
        qty: "Qty",
        price: "Price",
        value: "Value",
        pl: "P&L",
        triggered: "TRIGGERED",
        holding: "Holding",
        pastTrade: "Past Trade",
        geminiPending: "Gemini integration pending...",
        buy: "Buy",
        sell: "Sell",
        ticker: "Ticker",
        company: "Company",
        sector: "Sector",
        industry: "Industry",
        status: "Status",
        lastBuy: "Last Buy",
        lastSell: "Last Sell",
        totalPL: "Total P&L",
        change5d: "5D %",
        change20d: "20D %",
        change50d: "50D %",
        change200d: "200D %",
        filterAll: "All",
        filterHolding: "Holding",
        filterPast: "Past",
        filterTrending: "Trending (200MA)",
        searchPlaceholder: "Search Ticker, Company...",
        showingStocks: "Showing {{count}} of {{total}} stocks",
        noteSaved: "Note saved",
        dailyChart: "Daily",
        weeklyChart: "Weekly",
        viewOnTradingView: "View on TradingView",
        myNotes: "Notes",
        saveNote: "Save",
        geminiAnalysis: "AI Analysis",
        noAnalysis: "No Analysis",
        prev: "Prev",
        next: "Next",
        filterNotes: "Has Notes",
        refreshAnalysis: "Refresh Analysis",
        companyName: "Company",
        rate: "Rate",
        note: "Note",
        realizedPL: "Realized P&L",
        importedAt: "Imported At",
        saveView: "Save View",
        deleteView: "Delete View",
        columns: "Columns",
        custom: "Custom",
        advancedFilter: "Advanced Filter",
        analysis: "Analysis",
        save: "Save",
        cancel: "Cancel",
        apply: "Apply",
        visibleOrder: "Visible (Order)",
        available: "Available",
        confirmDeleteView: "Delete this view?",
        enterViewName: "Enter view name",
        deleteStock: "Delete Stock",
        confirmDeleteStock: "Are you sure you want to delete this stock? Notes and history will be removed.",
        copyPrompt: "Copy Prompt",
        generating: "Generating...",
        openReport: "Open Report",
        dev5: "Dev 5MA",
        dev20: "Dev 20MA",
        dev50: "Dev 50MA",
        dev200: "Dev 200MA",
        geminiPrompt: "Gemini Prompt",
        confirmStop: "Stop research?",
        failedToDelete: "Failed to delete",
        updateFailed: "Update failed",
        failedToOpenFile: "Failed to open file",
        failedToStop: "Failed to stop",
        chart: "Chart",
        chartLarge: "Chart (Large)",
        delete: "Delete",
        marketCap: "Market Cap",
        volume: "Volume",
        volIncrease: "Vol % Inc",
        lastEarnings: "Last Earnings",
        nextEarnings: "Next Earnings",
        // Deep Research
        deepResearchTitle: "Deep Research Automation",
        addToDeepResearch: "Add to Deep Research list",
        targetTickers: "Target Tickers (One per line)",
        clearList: "Clear List",
        confirmClearList: "Clear ticker list?",
        selectPrompt: "Select Prompt",
        editPrompts: "Edit Prompts",
        selectPromptPlaceholder: "Select a prompt...",
        startResearch: "START RESEARCH 🚀",
        starting: "Starting...",
        stopExecution: "STOP EXECUTION ⏹️",
        executionStatus: "Execution Status",
        ready: "Ready",
        processed: "Processed",
        currentTask: "Current Task",
        processing: "Processing: ",
        idle: "Idle",
        executionLogs: "Execution Logs",
        enterSymbolsAndPrompt: "Please enter symbols and select a prompt.",
        failedToStart: "Failed to start research",
        addedToResearch: "Added {{symbol}} to Deep Research list 🧠",
        indexAdded: "Index added successfully",
        failedToAddIndex: "Failed to add index",
        reportsUpdated: "Reports updated"
    },
    ja: {
        dashboardTitle: "投資ダッシュボード",
        automation: "自動化",
        runImport: "インポート実行",
        importing: "インポート中...",
        importStarted: "インポートを開始しました！",
        importCompleted: "インポート完了！",
        importFailed: "インポート失敗",
        importPlaceholder: "C:\\path\\to\\export.xls",
        noStocksFound: "銘柄が見つかりません。インポートを実行してください。",
        loading: "読み込み中...",
        stockNotFound: "銘柄が見つかりません",
        backToDashboard: "ダッシュボードに戻る",
        priceChart: "株価チャート",
        noChartData: "チャートデータなし",
        technicalSignals: "テクニカル指標",
        analysisNotes: "分析・メモ",
        tradeHistory: "取引履歴",
        noTradesFound: "取引履歴なし",
        date: "日付",
        side: "売買",
        qty: "数量",
        price: "価格",
        value: "金額",
        pl: "損益",
        triggered: "シグナル点灯",
        holding: "保有中",
        pastTrade: "過去取引",
        geminiPending: "Gemini連携準備中...",
        buy: "買い",
        sell: "売り",
        ticker: "銘柄コード",
        company: "社名",
        sector: "セクター",
        industry: "業種",
        status: "ステータス",
        lastBuy: "直近買付",
        lastSell: "直近売却",
        totalPL: "合計損益",
        change5d: "直近5日",
        change20d: "直近20日",
        change50d: "直近50日",
        change200d: "直近200日",
        filterAll: "すべて",
        filterHolding: "保有中",
        filterPast: "過去取引",
        filterTrending: "上昇トレンド(200日線)",
        searchPlaceholder: "銘柄コード、社名、セクターで検索...",
        showingStocks: "{{total}} 銘柄中 {{count}} 銘柄を表示",
        noteSaved: "メモを保存しました",
        dailyChart: "日足",
        weeklyChart: "週足",
        viewOnTradingView: "TradingViewで見る",
        myNotes: "メモ",
        saveNote: "保存",
        geminiAnalysis: "AI分析",
        noAnalysis: "分析レポートなし",
        prev: "前へ",
        next: "次へ",
        filterNotes: "メモ有",
        refreshAnalysis: "AI分析更新",
        companyName: "社名",
        rate: "レート",
        note: "メモ",
        realizedPL: "確定損益",
        importedAt: "取込日",
        saveView: "ビュー保存",
        deleteView: "ビュー削除",
        columns: "列設定",
        custom: "カスタム",
        advancedFilter: "詳細フィルタ",
        analysis: "AI分析",
        save: "保存",
        cancel: "キャンセル",
        apply: "適用",
        visibleOrder: "表示中 (順序)",
        available: "非表示",
        confirmDeleteView: "このビューを削除しますか？",
        enterViewName: "ビューの名前を入力",
        deleteStock: "銘柄を削除",
        confirmDeleteStock: "本当にこの銘柄を削除しますか？メモや履歴も全て削除されます。",
        copyPrompt: "プロンプトをコピー",
        generating: "生成中...",
        openReport: "レポートを開く",
        dev5: "乖離(5MA)",
        dev20: "乖離(20MA)",
        dev50: "乖離(50MA)",
        dev200: "乖離(200MA)",
        geminiPrompt: "Geminiプロンプト",
        confirmStop: "停止しますか？",
        failedToDelete: "削除に失敗しました",
        updateFailed: "更新に失敗しました",
        failedToOpenFile: "ファイルを開けませんでした",
        failedToStop: "停止に失敗しました",
        chart: "チャート",
        chartLarge: "チャート(大)",
        delete: "削除",
        marketCap: "時価総額",
        volume: "出来高",
        volIncrease: "出来高増加率",
        lastEarnings: "直近決算報告日",
        nextEarnings: "次回決算報告日",
        // Deep Research
        deepResearchTitle: "Deep Research 自動化",
        addToDeepResearch: "Deep Researchリストに追加",
        targetTickers: "対象銘柄 (1行に1つ)",
        clearList: "リストをクリア",
        confirmClearList: "リストをクリアしますか？",
        selectPrompt: "プロンプト選択",
        editPrompts: "プロンプト編集",
        selectPromptPlaceholder: "プロンプトを選択してください...",
        startResearch: "リサーチ開始 🚀",
        starting: "開始中...",
        stopExecution: "実行停止 ⏹️",
        executionStatus: "実行ステータス",
        ready: "待機中",
        processed: "処理済み",
        currentTask: "現在のタスク",
        processing: "処理中: ",
        idle: "アイドル",
        executionLogs: "実行ログ",
        enterSymbolsAndPrompt: "銘柄を入力し、プロンプトを選択してください。",
        failedToStart: "開始に失敗しました",
        addedToResearch: "Deep Researchリストに追加しました 🧠: {{symbol}}",
        indexAdded: "インデックスを追加しました",
        failedToAddIndex: "インデックスの追加に失敗しました",
        reportsUpdated: "レポート情報を更新しました"
    }
};

export function useTranslation() {
    const [lang, setLang] = useState<Lang>('ja'); // Default to JA or detect

    useEffect(() => {
        // Simple browser detection
        const browserLang = navigator.language;
        if (browserLang.startsWith('en')) {
            setLang('en');
        } else {
            setLang('ja');
        }
    }, []);

    function t(key: keyof typeof dictionary['en']) {
        return dictionary[lang][key] || dictionary['en'][key] || key;
    }

    return { t, lang, setLang };
}
